...
def test_parse_termux_location_tolerates_stray_whitespace():
    padded = f"\n\n  {SAMPLE_TERMUX_LOCATION}   \n"
    fix = _parse_termux_location(padded)
    assert fix is not None
    assert fix.lat == 22.4327


def test_parse_termux_location_extracts_json_from_stray_text():
    # Some devices/ROMs print an extra warning line around the JSON —
    # the parser should still find and use the real object.
    noisy = f"warning: some noisy line\n{SAMPLE_TERMUX_LOCATION}\n"
    fix = _parse_termux_location(noisy)
    assert fix is not None
    assert fix.lon == 87.8697
