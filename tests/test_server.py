...
def test_world_outline_is_served(running_server):
    status, data = _get_json("/world_countries.geojson")
    assert status == 200
    assert data["type"] == "FeatureCollection"
    assert len(data["features"]) > 100  # real countries, not a stub
    assert all("name" in f["properties"] for f in data["features"])


def test_static_assets_are_never_cached(running_server):
    with urllib.request.urlopen(BASE + "/", timeout=5) as resp:
        cache_control = resp.headers.get("Cache-Control", "")
    assert "no-store" in cache_control


def test_api_responses_are_never_cached(running_server):
    with urllib.request.urlopen(BASE + "/api/hazards", timeout=5) as resp:
        cache_control = resp.headers.get("Cache-Control", "")
    assert "no-store" in cache_control


def test_countries_endpoint_empty_by_default(running_server):
    status, data = _get_json("/api/countries")
    assert status == 200
    assert data["countries"] == []
    assert set(data["layers"]) == {"roads", "states", "districts", "forest", "water"}


def test_country_layer_serves_real_file(tmp_path, running_server, monkeypatch):
    import terralocator.server as server_module
    country_dir = tmp_path / "testcountry"
    country_dir.mkdir()
    sample = {"type": "FeatureCollection", "features": [
        {"type": "Feature", "geometry": {"type": "Point", "coordinates": [0, 0]}, "properties": {"name": "x"}}
    ]}
    (country_dir / "roads.geojson").write_text(json.dumps(sample), encoding="utf-8")
    monkeypatch.setattr(server_module, "COUNTRIES_DIR", tmp_path)

    status, data = _get_json("/api/country?name=testcountry&layer=roads")
    assert status == 200
    assert len(data["features"]) == 1


def test_country_layer_rejects_invalid_layer_name(running_server):
    with pytest.raises(urllib.error.HTTPError) as exc_info:
        _get_json("/api/country?name=x&layer=not_a_real_layer")
